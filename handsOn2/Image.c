...
int apply_filter_sequencial(int steps, int radius, struct pixel **src, struct pixel **dst, int width, int height)
{
  int i, j, k, l, s, total;
...
#pragma omp parallel for private(i, j)
  for (i = -radius; i <= radius; i++)
    for (j = -radius; j <= radius; j++)
      filter_block[i + radius][j + radius] = (radius - abs(i)) * (radius - abs(i)) + (radius - abs(j)) * (radius - abs(j)) + 1;
#pragma omp parallel for private(s, i, j, k, l, total, result, filter_factor)
  for (s = 0; s < steps; s++)
  {
...
int main(int argc, char **argv)
{
  ...
  int width, height;
  int i, result; 
  double afst0, afstf, afpt0, afptf, afst, afpt, speedUp;
  ...
  afst0 = omp_get_wtime();
  apply_filter_sequencial(NUM_STEPS, RADIUS, src_img, dst_img, width, height);
  afstf = omp_get_wtime();
  ...
  printf("Sequencial Time: %lf   Parallel Time: %lf   SpeedUp: %lf   Num Thread: %d\n", afst, afpt, speedUp, num_thread);
